...
### Requirement: Architecture config SHALL define non-recursive import allowlist selector semantics

Within a module policy, `imports` patterns SHALL match canonical target module paths. Exact patterns such as `foo` SHALL match only `foo`. Child wildcard patterns such as `foo/*` SHALL match only `foo/<one-segment>`. Self-or-child patterns such as `foo/+` SHALL match `foo` and `foo/<one-segment>`. Import allowlist patterns MUST remain non-recursive.

#### Scenario: Exact import pattern matches only exact module

- **WHEN** `imports` contains `models` and the target canonical module path is `models`
- **THEN** the pattern matches

#### Scenario: Exact import pattern does not match child module

- **WHEN** `imports` contains `models` and the target canonical module path is `models/a`
- **THEN** the pattern does not match

#### Scenario: Child wildcard import pattern matches direct child module

- **WHEN** `imports` contains `models/*` and the target canonical module path is `models/a`
- **THEN** the pattern matches

#### Scenario: Child wildcard import pattern does not match parent module

- **WHEN** `imports` contains `models/*` and the target canonical module path is `models`
- **THEN** the pattern does not match

#### Scenario: Child wildcard import pattern does not match deeper module

- **WHEN** `imports` contains `models/*` and the target canonical module path is `models/a/internal`
- **THEN** the pattern does not match

#### Scenario: Self-or-child import pattern matches parent module

- **WHEN** `imports` contains `models/+` and the target canonical module path is `models`
- **THEN** the pattern matches

#### Scenario: Self-or-child import pattern matches direct child module

- **WHEN** `imports` contains `models/+` and the target canonical module path is `models/a`
- **THEN** the pattern matches

#### Scenario: Self-or-child import pattern does not match deeper module
